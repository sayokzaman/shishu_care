...
function StatCard({
  ...
  return (
    <Animated.View
      ...
      style={{ flex: 1, backgroundColor: '#F5F5F5', borderRadius: 16, padding: 14 }}>
      <View
        style={{
          flexDirection: 'row',
      ...
      <Text style={{ fontSize: 12, fontWeight: '600', color: '#0A0A0A', marginTop: 1 }}>
        {label}
      </Text>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFFFF' }} edges={['top']}>
      <Header
        ...
        emoji="📈"
        secondaryText={`Dhaka District · Updated ${lastRefresh}`}>
        <Pressable
          onPress={() => setLastRefresh('Just now')}
          style={{
            width: 36,
            height: 36,
            borderRadius: 10,
      ...
      <ScrollView
        ...
        <Animated.View
          ...
          <StatCard
            label="Oxygen Units"
            value={`${oxygenUp}/${MOCK_DATA.length}`}
            sub="upazilas online"
            Icon={Droplets}
            color="#2563EB"
            delay={100}
          ...
          {(
            ...
            <Pressable
              ...
              style={{
                ...
                paddingVertical: 8,
                borderRadius: 10,
                backgroundColor: activeView === key ? '#0F5238' : 'transparent',
                alignItems: 'center',
              }}>
              <Text
                style={{
                  fontSize: 12,
                  fontWeight: '700',
                  color: activeView === key ? 'white' : '#737373',
                }}>
                {label}
              </Text>
            </Pressable>
        ...
        {/* Upazila list */}
        <Text style={{ fontSize: 13, fontWeight: '700', color: '#0A0A0A', marginBottom: 10 }}>
          Upazila Breakdown
        </Text>
        {MOCK_DATA.map((u, i) => (
          <Animated.View
            key={u.name}
            entering={FadeInDown.duration(400).delay(80 + i * 50)}
            style={{
              borderRadius: 14,
              borderWidth: 1,
              borderColor: '#E5E5E5',
            ...
            <View
                ...
                <View style={{ alignItems: 'flex-end' }}>
                  <Text
                    style={{
                      fontSize: 15,
                      fontWeight: '800',
                      color: u.bedsAvail < 5 ? '#DC2626' : '#0A0A0A',
                    }}>
                    {u.bedsAvail} free
                  </Text>
              ...
              {activeView === 'nutrition' && (
                <View
                  ...
                  {u.stunting > 35 ? (
                    ...
                    <TrendingDown size={14} color="#0A0A0A" />
                  )}
                  <Text
                    style={{
                      fontSize: 15,
                      fontWeight: '800',
                      color: u.stunting > 35 ? '#DC2626' : '#0A0A0A',
                    }}>
                    {u.stunting}%
                  </Text>
                </View>
              ...
              {activeView === 'vaccination' && (
                <View
                  ...
                  {u.vaccination >= 85 ? (
                    <TrendingUp size={14} color="#0A0A0A" />
                  ) : (
                  ...
                  <Text
                    style={{
                      fontSize: 15,
                      fontWeight: '800',
                      color: u.vaccination >= 85 ? '#0A0A0A' : '#D97706',
                    }}>
                    {u.vaccination}%
                  </Text>
                </View>
            ...
            {activeView === 'beds' && (
              <View>
                <View
                  ...
                  <Text style={{ fontSize: 11, color: '#737373', width: 32, textAlign: 'right' }}>
                    {Math.round(((u.beds - u.bedsAvail) / u.beds) * 100)}%
                  </Text>
                </View>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                  <View
                    style={{
                      ...
                      height: 8,
                      borderRadius: 4,
                      backgroundColor: u.oxygen ? '#0A0A0A' : '#DC2626',
                      marginLeft: 1,
                    }}
                  />
                  <Text
            ...
            {activeView === 'nutrition' && (
              <Text style={{ fontSize: 12, color: '#374151' }}>
                Under-5 stunting rate: <Text style={{ fontWeight: '700' }}>{u.stunting}%</Text> ·
                Active nutritional cases: {u.activeCases}
              </Text>
            )}
            {activeView === 'vaccination' && (
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                ...
                <View
                  ...
                  <View
                    style={{
                      height: '100%',
                      width: `${u.vaccination}%`,
                      backgroundColor: u.vaccination >= 85 ? '#0A0A0A' : '#D97706',
                      borderRadius: 4,
                    }}
                  />
                </View>
        ...
        <View style={{ backgroundColor: '#F5F5F5', borderRadius: 14, padding: 14, marginTop: 4 }}>
          <Text style={{ fontSize: 12, color: '#737373', lineHeight: 19 }}>
            Data shown is simulated for demonstration. In the deployed version, this dashboard
            connects to the DGHS (Directorate General of Health Services) real-time API. Bed
