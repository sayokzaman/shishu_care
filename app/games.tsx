  ...
  '3-5y': [
    {
      emojis: ['🦋', '🐛', '🥚'],
      answer: '2',
      question: "Which comes first in a butterfly's life?",
    },
        ...
        {selected !== null && (
          <Text
            style={{
              ...
              fontSize: 14,
              fontWeight: '700',
              color: selected === parseInt(puzzle.answer) ? '#0A0A0A' : '#DC2626',
            }}>
            {selected === parseInt(puzzle.answer)
              ? '🎉 Correct! Great job!'
              : `Not quite — the answer was ${puzzle.emojis[parseInt(puzzle.answer)]}!`}
          </Text>
        )}
      </View>
    </Animated.View>
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFFFF' }} edges={['top']}>
      ...
      <ScrollView
        ...
        <Animated.View entering={FadeInDown.duration(400).delay(100)}>
          <Text style={{ fontSize: 16, fontWeight: '800', color: '#0A0A0A', marginBottom: 4 }}>
            Puzzle Time
          </Text>
          <Text style={{ fontSize: 13, color: '#737373', marginBottom: 14 }}>
            Help your child point to the right answer!
          </Text>
          <PuzzleCard
            key={`${bracket}-${puzzleIdx}`}
          ...
          {[
            {
              emoji: '📚',
              title: 'Read Together',
              desc: 'Pick a picture book and name the objects on each page.',
              time: '10 min',
            },
            {
              emoji: '🎵',
              title: 'Sing a Nursery Rhyme',
              desc: 'Amar sonar bangla or Twinkle Twinkle — repetition builds language.',
              time: '5 min',
            },
            {
              emoji: '🖐️',
              title: 'Clapping Game',
              desc: 'Clap and copy rhythms together to develop motor coordination.',
              time: '5 min',
            },
            {
              emoji: '🌿',
              title: 'Nature Walk',
              desc: 'Point out colours, textures, and sounds during a short outdoor walk.',
              time: '20 min',
            },
          ].map((act, i) => (
            <Animated.View
              key={act.title}
              entering={FadeInDown.duration(400).delay(180 + i * 60)}
              style={{
                flexDirection: 'row',
                alignItems: 'flex-start',
                gap: 12,
